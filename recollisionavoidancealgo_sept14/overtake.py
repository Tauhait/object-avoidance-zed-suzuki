...
def callback_lane_state(data):
    global lane_state # 1 == GREEN, 0 == RED
...
def is_driving_lane_clear(driving_lane_space):
    global lane_state
    if lane_state == 0:
        if driving_lane_space > const.LANE_SPACE:
            lane_state_publish.publish(1)
            lane_state = 1
...
def drivespace(depth_map):
    ...
    if lane_state == const.DRIVING_LANE:
        ...
        if status == False or dist_to_free_lane_mid == None or np.isnan(dist_to_free_lane_mid):
            ...
            overtake_x, overtake_y = util.get_point_at_distance(dist_to_free_lane_mid, overtake_bearing, R=6371)
        ...
        if util.is_clear_to_switch(overtake_lane_space):
            collision_avoidance_publish.publish(const.SWITCH)

    
    return masked_image 

def main(device):
    global image_net, exit_signal, run_signal, detections, alternate_lane
    alternate_lane = 0
